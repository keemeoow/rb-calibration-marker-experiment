...
## ⚡ 요약 — 노이즈별 강건성 순위 (좋음 → 나쁨)

각 노이즈가 **커질수록** e_task(실전 정확도)가 **덜 나빠지는 순서**. `≈`는 사실상 동급, `≫`는 큰 격차.

| 노이즈 ↑ | 강건 순위 (좋음 → 나쁨) | Ours는? |
|---|---|---|
| **FK 오차** | −FK ≈ fixed-FK **>** −FK−통합 **>** Ours ≈ −보드 ≈ −통합 | ❌ **불리** (보정이 틀린 FK를 쫓음) |
| **오검출(outlier)** | **Ours** **>** −보드 **>** fixed-FK **>** −통합 **≫** −FK ≈ −FK−통합(붕괴) | ✅ **최고** |
| **계통(systematic)** | fixed-FK ≈ **Ours** ≈ −FK ≈ −보드 (통합 계열) **>** −통합 ≈ −FK−통합 (독립 계열) | ✅ **상위** |

**한 줄 정리**: **FK가 정확하면**(사용자 실제 목표) → **오검출·계통에서 Ours가 최고**. 단 **순수 FK 오차엔 Ours가 불리**(FK 안 쓰는 계열이 유리). `−큐브(EXP6)`는 캘리브 붕괴라 순위에서 제외.
...
| EXP6 −FK-큐브 | 0.00 | ~~6.51~~ | ~~0.66~~ | 45.62 |
