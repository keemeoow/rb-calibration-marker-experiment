...
| EXP4 | VISION | 통합 | 큐브+보드 | VISION |
| EXP5 | VISION | 따로 | 큐브+보드 | VISION−통합 |
| EXP6 | VISION | 통합 | 보드만 | −큐브 |
| EXP7 | FK hard fixed | 통합(=따로) | 큐브+보드 | FK 고정 |
...
| **계통노이즈** (intrinsic 편향) | corrected-FK ≈ FK hard fixed 최저 | 순수 계통에는 FK hard fixed도 경쟁력 있음 |
| **FK 오차** (랜덤) | **Ours 최악** | 보정이 틀린 FK를 쫓음 — 정직한 약점 (FK≈0이면 무관) |
| **오검출** (마커 misdetection) | **Ours 최저, VISION 붕괴** | ⭐ 핵심 강점 |
...
| EXP4 VISION | 25.90 | 7.26 | 73.88 | 85.46 | 19.56 | 97.65 |
| EXP5 VISION−통합 | 29.49 | 12.59 | **44.89** | **39.93** | 40.59 | **75.04** |
...
| EXP4 VISION | 0.00 | 25.90 | **1.22** | 114.78 |
| EXP5 VISION−통합 | 0.00 | 29.49 | 2.69 | 96.60 |
| EXP6 −큐브 | 0.00 | ~~6.51~~ | ~~0.66~~ | 45.62 |
| EXP7 FK hard fixed | 0.00 | 26.55 | 1.26 | 51.95 |

- **현실종합·오검출 열 → Ours 최고** (유효 방법 중)
- **FK오차 열 -> Ours 약점** (VISION이 유리) -> **"FK 정확" 전제 명시 필요**
...
| FK hard fixed | 28.0 | 26.7 | 26.1 | 25.3 | 27.5 | 28.6 |
| VISION | 26.9 | 21.7 | 23.8 | 20.6 | 21.7 | 22.1 |
| −통합(indep) | 102.2 | 28.3 | 40.1 | 21.9 | 17.6 | 26.2 |

**핵심 발견**:
- **Ours만 셋↑에 크게 수렴** (26.8 → ~10mm, 6셋부터 최저) — FK 잔차보정이 셋이 많을수록 더 잘 학습됨.
- **FK hard fixed는 평평(~27mm)** — 데이터를 늘려도 안 좋아짐. 오차가 **편향(계통+오검출)** 지배라, 데이터로 못 줄임.
- **VISION도 평평(~22mm)**, −통합은 소량 셋에서 불안정(102mm).
