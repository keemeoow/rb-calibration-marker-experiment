...
Ours (EXP1) = 통합 BA + 큐브·보드 동시 + **공분산 가중 robust corrected-FK soft factor**.
...
절제 논리 (EXP1 에서 하나씩 제거):
  EXP1 Ours (큐브+보드 · 통합 · corrected-FK soft factor)
...
EXP1 = ExpConfig("EXP1", fk="factor", solve="unified",     markers=CB, label="Ours (통합·큐브+보드·corrected-FK soft factor)")
...
EXP4 = ExpConfig("EXP4", fk="none",   solve="unified",     markers=CB, label="VISION")
EXP5 = ExpConfig("EXP5", fk="none",   solve="independent", markers=CB, label="VISION −통합")
EXP6 = ExpConfig("EXP6", fk="none",   solve="unified",     markers=B,  label="−큐브 (보드만)")
EXP7 = ExpConfig("EXP7", fk="fixed",  solve="unified",     markers=CB, label="FK hard fixed")
