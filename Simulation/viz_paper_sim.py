...
def tables(blob):
    ...
    lines = ["## 표 1 — 시뮬 · 현실 종합 조건 (GT)\n",
             f"*{blob['meta']['protocol']}, {blob['meta']['seeds']} seeds. 실측노이즈: σ0.2 + 계통0.5% + FK≈0 + 오검출2%@2px.*",
             "*EXP6(-cube)은 e_X/reproj/cross 붕괴 → e_task 낮은 건 그리퍼예측 착시(캘리브 실패).*\n",
             "| 방법 | e_task mm | e_task deg | e_X mm | **e_rel mm** (상대정합) | gTc mm | reproj px | cross mm |",
             "|---|--:|--:|--:|--:|--:|--:|--:|"]
    for m in METH:
        r = R[rk][m]
        def g(k): return f"{r[k]:.2f}" if r.get(k) is not None else "—"
        lines.append(f"| {m} {LAB[m]} | {g('e_task_mm')} | {g('e_task_deg')} | "
                     f"{g('e_X_mm')} | {g('e_rel_mm')} | {g('gTc_mm')} | {g('e_reproj_px')} | {g('e_cross_mm')} |")
    # 표 1b: e_task 조건별
