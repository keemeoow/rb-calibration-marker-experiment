...
\begin{frame}{9.1 방법 1: No-FK}
  ...
  \vfill
  \begin{itemize}
    \item argmin 아래 = \textbf{움직여도 되는 값}. 큐브 자세가 여기 들어가 있음
    \item 오른쪽 $\equiv$ 는 제약이 아니라 \textbf{같은 것을 가리키는 두 이름}
      \begin{itemize}
        \item 8장의 기준 자세 자리를 자유변수가 그대로 채운다는 뜻
        \item \texttt{No-FK}에서는 \textbf{못 박는 값이 하나도 없음}
          (10.2절, 11.8절과 다른 점)
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{10.2 목적함수}
  \[
    \underset{\{\T{B}{C_i}\},\ \T{G}{C_g}}{\operatorname{argmin}}
    \ \mathcal{E}_{\mathrm{vis}}
    \qquad\text{단}\qquad
    \T{B}{\Oref_s}=\T{B}{\Ofk_s}
  \]
  ...
  \vfill
  \begin{itemize}
    \item argmin 아래에서 큐브 자세가 빠짐 $\rightarrow$ 미지수가 둘로 줄어듦
  \end{itemize}
\end{frame}
...
\begin{frame}{11.8 단계 7: anchor 고정 후 최종 refinement}
  ...
  \]
  \begin{itemize}
    \item 식의 형태는 10.2절 \texttt{Fixed-FK}와 동일
    \item 다른 것은 못 박은 값뿐:
