...
\begin{frame}{1.4 구성 1: eye-in-hand (카메라가 손에 붙음)}
  ...
  \vfill
  \centering
  카메라가 물체에 가까이 갈 수 있음. 대신 한 번에 좁은 범위만 봄
\end{frame}
...
\begin{frame}{1.5 구성 2: eye-to-hand (카메라가 밖에 고정)}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{어떻게 푸는가}
        \begin{minipage}[t][20mm][t]{\linewidth}
          \begin{itemize}
            \item 로봇 손에 마커를 들려 여러 자세로 움직임
            \item 마커 위치를 로봇도 알고 카메라도 봄 $\rightarrow$ 맞춰봄
          \end{itemize}
        \end{minipage}
      \end{block}
    \end{column}
  ...
  \vfill
  \centering
  작업 공간 전체를 봄. 대신 멀어서 정밀도가 떨어지고 로봇에 가려짐
\end{frame}

\begin{frame}{1.6 구성 3: eye-to-hand 여러 대}
  가림과 사각지대를 줄이려고 고정 카메라를 여러 대 설치
  \vfill
  \begin{alertblock}{순진한 방법: 카메라마다 따로 캘리브레이션}
    카메라 $0$은 카메라 $0$대로, 카메라 $1$은 카메라 $1$대로 각각 품\\
    $\rightarrow$ 같은 물체를 두고 \textbf{서로 다른 좌표}를 말하게 됨 (1.3절)
  \end{alertblock}
  \vfill
  \begin{block}{그래서 한꺼번에 푼다}
    \begin{itemize}
      \item 모든 카메라가 \textbf{같은 물체를 동시에} 보게 함
      \item 모두가 같은 답을 말하도록 카메라 위치들을 \textbf{함께} 조정
      \item 새로 생기는 미지수: \textbf{그때 물체가 어디 있었는가}
    \end{itemize}
  \end{block}
  \vfill
  \centering
  {\small 카메라를 따로 세우면 어긋남이 남는다 $\rightarrow$ 묶어서 세워야 한다}
\end{frame}

\begin{frame}{1.7 구성 4: 여기에 eye-in-hand까지 추가}
  고정 카메라 여러 대 \textbf{+} 손목 카메라 하나 = 이 프로젝트의 구성
  \vfill
  \begin{block}{손목 카메라를 왜 더하는가}
    \begin{itemize}
      \item 로봇과 함께 움직이며 \textbf{여러 각도에서} 같은 물체를 봄
      \item 고정 카메라 A와 B가 서로 안 보여도,
        손목 카메라가 양쪽을 차례로 방문 $\rightarrow$ \textbf{둘을 이어주는 다리}
      \item 물체에 가까이 가서 찍으므로 정밀한 관측을 보탬
    \end{itemize}
  \end{block}
  \vfill
...
\begin{frame}{4.1 -- 4.3 관측값의 정의}
  ...
  \begin{block}{}
    \centering
    앞의 둘은 출발점이 \textbf{카메라}, 마지막은 출발점이 \textbf{베이스}\\
    $\rightarrow$ 그대로는 서로 이어붙일 수 없음 (6장에서 연결)
  \end{block}
\end{frame}
...
\begin{frame}{9.2 장점과 주의점}
  ...
  \begin{block}{주의점}
    \begin{itemize}
      ...
      \item $\T{B}{G,e}$ \textbf{는 사용함}.
        큐브 FK prior $\T{B}{\Ofk_s}$ 만 사용하지 않음
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{10. 방법 2: Fixed-FK}
  ...
  \end{block}
  \vfill
  raw FK가 말하는 큐브 자세: $\T{B}{\Ofk_s}$
  \begin{itemize}
    \item 카메라를 전혀 쓰지 않고 로봇의 관절 센서만으로 얻는 큐브 자세
    \item 좌표계 이름에 $\mathrm{fk}$를 붙여 vision이 말하는 큐브와 구분
  \end{itemize}
  \vfill
  기준 자세를 여기에 못 박음
  \[
...
\begin{frame}{10.4 카메라에도 계통오차가 있으면 (3) 분리 불가}
  ...
  \begin{block}{\texttt{No-FK}와 비교}
    \begin{itemize}
      ...
      \item \texttt{Fixed-FK}: FK 오차와 카메라 오차가 모두
        $\T{B}{C_i}$, $\T{G}{C_g}$ 로 유입
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{11. 정리: 두 방식의 비교}
  \begin{center}
    ...
    \begin{tabular}{lll}
      ...
      raw FK 오차 & 들어오지 않음 & $\T{B}{C_i}$, $\T{G}{C_g}$ 로 유입 \\
      카메라 계통오차 & 들어옴 & 들어옴 \\
      오염원 & \textbf{하나} & \textbf{둘} \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vfill
  \begin{alertblock}{현재 선택}
    \begin{itemize}
      \item FK가 정확하다고 보장할 수 없고,
        틀려도 잔차로는 확인되지 않음 (10.4절)
      \item $\rightarrow$ 오염원이 하나뿐인 \texttt{No-FK}를 기준으로 삼음
    \end{itemize}
  \end{alertblock}
  \vfill
  \centering
  {\small raw FK를 보정해서 쓰는 방식은 검증 진행 중 $\rightarrow$ 별도 발표}
\end{frame}
\fi
