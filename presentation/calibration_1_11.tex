...
\begin{frame}{11.7 탈락한 세트는 \texttt{No-FK}와 같은가?}
  값의 출처가 vision이라는 점은 같지만, 같지는 않다.
  \vfill
  \begin{center}
    \begin{tabular}{cll}
      \toprule
       & 큐브 자세의 값 & 최적화 중 상태 \\
      \midrule
      \texttt{No-FK} & vision으로 찾는다 & 자유변수. 계속 움직인다 \\
  ...
  \vfill
  \begin{block}{정리}
    \texttt{Corrected-FK}는 세트마다 anchor의 \textbf{출처}가 갈릴 뿐,
    어느 쪽이든 값을 하나로 못 박고 시작한다는 성격은 같다.
    통과한 세트는 보정된 FK에, 탈락한 세트는 vision 합의에 고정된다.
    그 고정된 anchor로 다음 단계의 refinement를 수행한다.
  \end{block}
\end{frame}
