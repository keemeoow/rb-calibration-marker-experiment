...
\begin{frame}{11.6 기준선은 상수가 아님}
  ...
  \begin{itemize}
    ...
    \item $k=2.5$ = 중앙값에서 표준편차 $2.5$배까지 정상으로 인정
      \begin{itemize}
        \item $\overline{\boldsymbol{\Delta}}$를 구하는 강건 평균이 이미 쓰는 값과 동일
      \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}{11.6 왜 분포 기준으로 정하는가}
  $\boldsymbol{\Delta}_s = \overline{\boldsymbol{\Delta}}$이면
  $\mathbf{F}^{\mathrm{corr}}_s=\mathbf{V}_s$ $\rightarrow$ $d_t(s)=0$
  \vfill
  $\Rightarrow$ $d_t(s)$는 FK가 절대적으로 얼마나 틀렸는지가 아니라,\\
  \textbf{그 세트가 다른 세트들의 공통 경향에서 얼마나 벗어났는지}를 재는 값
  \vfill
  \begin{block}{그렇다면 얼마부터 유별난 값인가}
    나머지 세트들이 얼마나 모여 있느냐에 따라 달라짐
    \begin{itemize}
      \item 세트들이 모두 $2\,\mathrm{mm}$ 안에 모임
        $\rightarrow$ $10\,\mathrm{mm}$짜리 세트는 명백히 이상
      \item 원래부터 $30\,\mathrm{mm}$씩 흩어짐
        $\rightarrow$ 같은 $10\,\mathrm{mm}$도 평범
    \end{itemize}
...
\begin{frame}{11.6 하한을 무엇으로 정하는가}
  상수를 다시 쓰면 처음 문제로 회귀 $\rightarrow$ \textbf{vision 합의 자신의 흔들림}을 사용
  \[
    \tau^{\min}_t
    =
  ...
  \begin{block}{이유: 분해능}
    \begin{itemize}
      \item 카메라들끼리 $1.5\,\mathrm{mm}$씩 어긋남
        $\rightarrow$ $\mathbf{V}_s$ 자체가 그만큼 불확실
      \item 이때 FK가 $1\,\mathrm{mm}$ 벗어남
        $\rightarrow$ FK 탓인지 vision 탓인지 구별 불가
      \item 자의 눈금보다 작은 차이를 재려는 것과 같음
      \item $\rightarrow$ 구별할 수 없는 차이로 탈락시키는 것은 근거 없음
    \end{itemize}
  \end{block}
\end{frame}
