...
\begin{frame}{9. 앞으로 볼 세 가지 방식}
  ...
  \end{itemize}
  \vfill
  {\small 어깨의 $\mathrm{fk}$, $\mathrm{anc}$ 는 \textbf{같은 큐브를 누가 어떻게 짚었는가}를 구분하는 이름.
...
\begin{frame}{9.1 방법 1: No-FK}
  ...
  \[
    \begin{aligned}
      \text{최소화} \quad & \mathcal{E}_{\mathrm{vis}}
      \\[3pt]
      \text{움직이는 값} \quad &
        \{\T{B}{C_i}\},\quad \T{G}{C_g},\quad \{\T{B}{O,s}\}
      \\[3pt]
      \text{기준 자세} \quad &
        \T{B}{\Oref_s}\;\equiv\;\T{B}{O,s}
    \end{aligned}
  \]
  \vfill
  \begin{itemize}
    \item 둘째 줄에 \textbf{큐브 자세가 들어가 있음} $\rightarrow$ 함께 찾는 대상
    \item 셋째 줄의 $\equiv$ 는 제약이 아니라 \textbf{같은 것을 가리키는 두 이름}
      $\rightarrow$ \texttt{No-FK}에는 못 박는 값이 하나도 없음
  \end{itemize}
\end{frame}
...
\begin{frame}{10.2 목적함수}
  \[
    \begin{aligned}
      ...
      \\[3pt]
      \text{움직이는 값} \quad &
        \{\T{B}{C_i}\},\quad \T{G}{C_g}
        \qquad\text{\small(큐브 자세가 빠짐)}
      \\[3pt]
      \text{기준 자세} \quad &
        \T{B}{\Oref_s}=\T{B}{\Ofk_s}
        \qquad\text{\small(상수. 못 박음)}
    \end{aligned}
  \]
  \vfill
  완전히 펼치면
  ...
  \vfill
  \begin{itemize}
    \item 9.2절과 비교하면 \textbf{둘째 줄에서 큐브 자세가 사라짐}
      $\rightarrow$ 미지수가 둘로 줄어듦
  \end{itemize}
\end{frame}
...
\begin{frame}{11.8 단계 7: anchor 고정 후 최종 refinement}
  확정된 $\T{B}{\Oanc_s}$ 를 고정 anchor로 사용
  \[
    \begin{aligned}
      \text{최소화} \quad & \mathcal{E}_{\mathrm{vis}}
      \\[3pt]
      ...
      \\[3pt]
      \text{기준 자세} \quad &
        \T{B}{\Oref_s}=\T{B}{\Oanc_s}
        \qquad\text{\small(상수. 못 박음)}
    \end{aligned}
  \]
  \begin{itemize}
    \item 세 줄 모두 10.2절 \texttt{Fixed-FK}와 같은 자리, 같은 형태
    \item 다른 것은 셋째 줄에 못 박은 값뿐:
