...
\begin{frame}{1.1 로봇과 카메라는 서로 다른 것을 안다}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{로봇이 아는 것}
        \begin{minipage}[t][26mm][t]{\linewidth}
          관절이 몇 도 꺾여 있는지 안다.\\
          그래서 \textbf{자기 손끝이 어디 있는지} 계산할 수 있다.
          \vspace{2mm}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{카메라가 아는 것}
        \begin{minipage}[t][26mm][t]{\linewidth}
          \textbf{물체가 어디 있는지} 볼 수 있다.
          \vspace{2mm}

          그러나 자기 렌즈 앞을 기준으로만 말한다.\\
          ``내 앞 $60\,\mathrm{cm}$, 오른쪽으로 $10\,\mathrm{cm}$''
        \end{minipage}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{alertblock}{캘리브레이션이란?}
    둘 사이의 번역기를 만드는 일\\
    카메라가 말한 위치를 로봇이 움직일 수 있는 좌표로 바꾸는 규칙을 찾는다.
  \end{alertblock}
\end{frame}
...
\begin{frame}{1.2 왜 중요한가}
  ...
  \vfill
  \begin{block}{예시}
    카메라가 물체를 $1\,\mathrm{mm}$ 오차로 찾아냈다.
    그런데 카메라와 로봇 사이의 관계가 $10\,\mathrm{mm}$ 틀어져 있다.
    \vspace{2mm}

    $\Rightarrow$ 로봇이 실제로 가는 곳은 $10\,\mathrm{mm}$ 빗나간다.
    인식 성능을 아무리 올려도 이 오차는 줄지 않는다.
  \end{block}
  \vfill
  \begin{alertblock}{}
    캘리브레이션 정확도가 전체 시스템 정확도의 \textbf{상한}을 정한다.
  \end{alertblock}
\end{frame}

\begin{frame}{1.3 카메라가 여러 대면 문제가 하나 더 생긴다}
  ...
  \begin{block}{그래서 해야 할 일}
    ...
    그래야 여러 카메라가 같은 물체에 대해 같은 좌표를 말할 수 있다.
  \end{block}
\end{frame}

\begin{frame}{1.4 이 프로젝트의 설정}
  로봇 그리퍼가 큐브를 잡고 여러 위치에 옮겨 놓으면서 관측을 모은다.\\
  큐브를 한 번 놓은 상태를 \textbf{세트}라고 부른다.
  \vfill
  관측하는 카메라는 두 종류다.
  \begin{itemize}
    \item \textbf{고정 카메라}: 작업 공간 주변에 고정되어 있다. 여러 대를 쓴다.
    \item \textbf{그리퍼 카메라}: 로봇 손목에 붙어 로봇과 함께 움직인다.
  \end{itemize}
  \vfill
  \begin{alertblock}{이 문서 후반부의 핵심 주제}
    로봇이 큐브를 잡고 있으므로 로봇의 관절값으로부터 큐브가 어디 있는지 짐작할 수 있다.\\
    이 정보를 \textbf{얼마나 믿을지}가 9장부터 11장까지의 주제다.
  \end{alertblock}
\end{frame}
  ...
  \begin{block}{역행렬을 쓰는 이유}
    \begin{enumerate}
      ...
      \item 두 자세가 얼마나 다른지 잰다. 자세끼리는 뺄셈이 성립하지 않으므로,
        한쪽에서 다른 쪽까지 얼마나 더 움직여야 하는지를 대신 구한다.
...
\begin{frame}{4. 카메라가 실제로 측정하는 것은 무엇인가?}
  ...
  \vfill
  PnP(Perspective-$n$-Point). \\한 장의 사진만 가지고 물체가
  카메라 기준으로 어디에 어떤 방향으로 놓여 있는지를 알아내는 방법이다.
  \vfill
  \begin{block}{PnP를 풀려면 필요한 두 가지 정보}
    \begin{enumerate}
...
\begin{frame}{앞으로 볼 세 가지 방식}
  ...
  \vfill
  \begin{itemize}
    \item \texttt{No-FK}: 큐브 자세도 직접 찾는다. \hfill (9장)
    \item \texttt{Fixed-FK}: 큐브 자세를 raw FK에 못 박는다. \hfill (10장)
    \item \texttt{Corrected-FK}: raw FK를 vision으로 보정하고
      gate를 통과한 세트에서만 사용한다. \hfill (11장)
  \end{itemize}
  \vfill
  {\small $\mathbf{F}_s$는 10.1절, $\mathbf{A}_s$는 11.7절에서 정의한다.
  여기서는 세 방식이 같은 자리를 서로 다른 값으로 채운다는 점만 보면 된다.}
\end{frame}
...
\begin{frame}{11.3 강건 평균이 이상치를 거르는 기준}
  현재 평균을 기준으로 각 예측이 얼마나 떨어져 있는지를 재고,
  그 거리들의 분포에서 잘라낼 선을 정한다.
  \[
    \text{버린다}
    ...
    \qquad k=2.5
  \]
  걸러낸 뒤 남은 값으로 평균을 다시 내고,
  더 걸러낼 것이 없을 때까지 최대 세 번 반복한다.
  이상치를 한 번 제거하면 평균이 조금 옮겨가고,
  그 새 평균을 기준으로 다시 재면 처음에 놓쳤던 이상치가 드러나기 때문이다.
  \vfill
  \begin{block}{11.6절의 gate도 같은 형태의 식을 쓴다}
    \begin{center}
      \begin{tabular}{ll}
        \toprule
         & 무엇들의 거리를 재는가 \\
...
\begin{frame}{11.6 두 가지 차이를 재는 식}
  ...
  \vfill
  \vfill
  이동 차이 앞의 $1000$은 단위 환산이다. 자세의 위치는 미터로 다루므로
  $1000$을 곱해 밀리미터로 바꾼다. 회전 차이 앞의 $180/\pi$도 같은 이유로
  라디안을 도로 바꾼다.
  \vfill
...
\begin{frame}{11.7 단계 6: gate 결과에 따라 anchor 확정}
  ...
  \end{center}
  \vfill
  통과한 세트는 보정된 FK를 \textbf{그대로} anchor로 삼고,
