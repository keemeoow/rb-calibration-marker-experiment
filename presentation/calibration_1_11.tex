...
\begin{frame}{9.1 방법 1: No-FK}
  ...
  \vfill
  \begin{itemize}
    \item 큐브 자세를 미리 정해주지 않고 \textbf{찾아야 할 값}으로 남겨둠
    \item $\mathcal{E}_{\mathrm{vis}}$ 를 최소화하며 찾는 값은
      \textbf{세 덩어리}
      \begin{itemize}
        \item $\{\T{B}{C_i}\}$, \quad $\T{G}{C_g}$, \quad $\{\T{B}{O,s}\}$
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{10.2 목적함수}
  ...
  \]
  \begin{itemize}
    \item 최소화하며 찾는 값은 \textbf{두 덩어리}:
      \ $\{\T{B}{C_i}\}$, \quad $\T{G}{C_g}$
    \item 9.2절과 달리 큐브 자세가 빠짐 $\rightarrow$ 미지수가 줄어듦
  \end{itemize}
  \vfill
  완전히 펼치면
  {\footnotesize
...
\begin{frame}{11.8 단계 7: anchor 고정 후 최종 refinement}
  ...
  \begin{itemize}
    \item 최소화하며 찾는 값은 \textbf{두 덩어리}:
      \ $\{\T{B}{C_i}\}$, \quad $\T{G}{C_g}$ \quad (10.2절과 동일)
    \item 다른 것은 넣은 값뿐:
