...
\begin{frame}{9.1 방법 1: No-FK}
  ...
  \vfill
  \begin{itemize}
    \item 세 번째 자리에 \textbf{자유변수} $\{\T{B}{O,s}\}$ 를 넣음
    \item 그 값이 $\min$ 아래에도 그대로 있음
      $\rightarrow$ \textbf{큐브 자세까지 함께 찾는다}는 뜻
  \end{itemize}
\end{frame}
...
\begin{frame}{10.2 목적함수}
  ...
  \vfill
  \begin{itemize}
    \item 세 번째 자리에 \textbf{raw FK 값} $\{\T{B}{\Ofk_s}\}$ 를 넣음
    \item 9.2절과 달리 그 값이 $\min$ 아래에는 \textbf{없음}
      $\rightarrow$ 못 박혀서 움직이지 못함
  \end{itemize}
\end{frame}
...
\begin{frame}{11.8 단계 7: anchor 고정 후 최종 refinement}
  ...
  \begin{itemize}
    \item 10.2절 \texttt{Fixed-FK}와 \textbf{글자 하나까지 같은 형태}
    \item 다른 것은 세 번째 자리에 넣은 값뿐:
