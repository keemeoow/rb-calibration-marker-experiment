...
\begin{frame}{11.6 왜 분포 기준으로 정하는가}
  \begin{block}{어떤 세트의 어긋남이 공통 치우침과 정확히 같다면}
    \begin{enumerate}
      \item $\boldsymbol{\Delta}_s = \overline{\boldsymbol{\Delta}}$
        (이 세트의 어긋남 = 전체 평균 어긋남)
      \item $\rightarrow$ 평균만큼 보정하면 정확히 vision 자리에 도착:
        $\mathbf{F}^{\mathrm{corr}}_s=\mathbf{V}_s$
      \item $\rightarrow$ vision과의 거리가 $0$: $d_t(s)=0$
    \end{enumerate}
    \vspace{1mm}
    (시계 비유: $5$분 빠른 시계에서 $5$분을 빼면 정확히 맞음)
  \end{block}
  \vfill
  $\Rightarrow$ $d_t(s)=0$은 FK가 정확했다는 뜻이 아님.
  \textbf{남들만큼만 틀렸다}는 뜻\\
  $\Rightarrow$ $d_t(s)$가 재는 것 =
