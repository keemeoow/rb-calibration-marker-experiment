...
\begin{frame}{11.6 왜 분포 기준으로 정하는가}
  \begin{block}{어떤 세트의 어긋남이 공통 치우침과 같다면}
    \begin{enumerate}
      ...
      \item $\rightarrow$ 두 값이 겹침:
        $\mathbf{F}^{\mathrm{corr}}_s=\mathbf{V}_s$, 따라서 $d_t(s)=0$
    \end{enumerate}
    \vspace{1mm}
    (시계 비유: $5$분 빠른 시계에서 $5$분을 빼면 남는 오차가 없음)
  \end{block}
  \vfill
  \begin{alertblock}{$d_t(s)=0$이 뜻하지 않는 것}
    \begin{itemize}
      \item FK가 정확했다는 뜻이 아님 $\rightarrow$ \textbf{남들만큼만 틀렸다}는 뜻
      \item 그 자리가 참값이라는 뜻도 아님 $\rightarrow$ 두 값이 겹쳤을 뿐
      \item $\Rightarrow$ $d_t(s)$가 재는 것 =
        \textbf{그 세트가 공통 경향에서 얼마나 벗어났는가}
    \end{itemize}
  \end{alertblock}
\end{frame}

\begin{frame}{11.6 그렇다면 얼마부터 유별난 값인가}
  ...
  \begin{alertblock}{}
    ...
    $\rightarrow$ 기준선을 데이터의 산포에서 가져오는 이유
  \end{alertblock}
\end{frame}
