...
이 실험은 canonical Table 1을 덮어쓰지 않는다. Session04 manifest, split, solver, A4 row를 고정하고 Simulation prior covariance의 표준편차 scale만 바꾼다. `preflight_std_scale < 1`은 corrected-FK soft factor를 더 강하게, `> 1`은 더 약하게 넣는다는 뜻이다.

> External GT를 사용하지 않았으므로 이 결과는 물리 정확도 우월성 근거가 아니라 corrected-FK soft factor 영향도 점검이다.
...
- 따라서 현재 데이터에서는 corrected-FK soft factor가 완전히 무시된다고 보기는 어렵지만, A2 대비 A4의 내부 held-out 차이는 매우 작아 최종 우월성 claim으로 쓰기에는 부족하다.
