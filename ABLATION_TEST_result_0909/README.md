...
상세 보고서는 [`session04/ABLATION_TEST_table1/ABLATION_TEST_TABLE1_RESULTS.md`](session04/ABLATION_TEST_table1/ABLATION_TEST_TABLE1_RESULTS.md), interactive 결과는 [`session04/ABLATION_TEST_table1/ABLATION_TEST_TABLE1_INTERACTIVE.html`](session04/ABLATION_TEST_table1/ABLATION_TEST_TABLE1_INTERACTIVE.html)에서 확인한다. 추가 진단/민감도 실험은 루트의 [`ADD_EXPERIMENTS_SUMMARY.md`](../ADD_EXPERIMENTS_SUMMARY.md) 한 곳에서 요약한다.
...
[`ABLATION_TEST_TABLE1_RESULTS.md`](session04/ABLATION_TEST_table1/ABLATION_TEST_TABLE1_RESULTS.md) 한 곳에서 확인한다.
...
`session04/ABLATION_TEST_table1/ABLATION_TEST_table1_methods.json`은 A0~A5/B1~B3의 유일한 원시 실행 결과다. A6나 추가 board-only FK row는 최종 표에 넣지 않는다. `ABLATION_TEST_table1_results.csv`, `ABLATION_TEST_TABLE1_RESULTS.md`, `ABLATION_TEST_TABLE1_INTERACTIVE.html`은 이 JSON과 보조 평가 JSON에서 생성하되, 최종 출력은 cube-only 지표만 남긴다.
