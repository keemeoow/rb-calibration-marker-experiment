...
A5는 corrected-FK hard fixed (train-only VISION alignment) 최종 후보가 될 수 있다. 단, 이 sensitivity 결과만으로 A5를 외부 물리 정확도 winner로 확정하지는 않으며, GT 공개 전에 방법과 alignment artifact를 frozen해야 한다.
