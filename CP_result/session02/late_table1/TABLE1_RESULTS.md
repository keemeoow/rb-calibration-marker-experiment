...
- A4/B1/B2 preflight: 실측 covariance 대신 Simulation과 동일하게 동결한 `sigma_t=2 mm`,
  `sigma_R=0.3 deg` 등방성 prior와 FK Huber `f_scale=3.0`을 사용했다. 따라서
  confirmatory-ready는 `false`다.
...
| A4 | 3/3 | 3 | 3.0959 / 2.5038 / 4.4556 | 13.0413 / 1.1899 | 4.0330 / 1.0313 | preflight |
| A5 | — | — | — | — | — | 미구현; 6-DoF correction label·외부 GT 없음 |
| B1 | 3/3 | 3 | 3.1057 / 2.5100 / 4.4724 | 13.6110 / 1.2157 | 4.1987 / 1.0184 | preflight |
| B2 | 3/3 | 3 | 4.4759 / — / 4.4759 | 11.9161 / 1.2866 | **3.8215** / **0.9311** | preflight |
...
| A4 | 1.6034 | 2.8522 | 3.3979 | 3.1854 |
| B1 | 1.6101 | 2.9089 | 3.4010 | 3.2054 |
| B2 | 3.2760 | 4.4746 | 4.5786 | 5.1232 |
...
| A2→A4 | overall reproj / `e_cross_t` / `e_e2e_t` | -0.0195 px / -0.0863 mm / -0.0363 mm |
| B1→A4 | overall reproj / `e_cross_t` / `e_e2e_t` | -0.0097 px / -0.5697 mm / -0.1657 mm |
| B2→A4 | cube reproj / `e_cross_t` / `e_e2e_t` | -0.0203 px / +1.1252 mm / +0.2115 mm |
...
| --- | ---: | ---: | ---: |
| A4a Simulation-isotropic linear | 3.0959 / 2.5038 / 4.4557 | 13.0367 / 1.1901 | 4.0303 / 1.0311 |
| A4b covariance linear | 3.0959 / 2.5038 / 4.4557 | 13.0367 / 1.1901 | 4.0303 / 1.0311 |
| A4c=A4 covariance robust | 3.0959 / 2.5038 / 4.4556 | 13.0413 / 1.1899 | 4.0330 / 1.0313 |

실측 covariance가 없어 A4a와 A4b는 같은 Simulation 대각 prior를 사용했고 결과도 동일하다.
A4b→A4 robustification의 변화는 reprojection `-0.0000 px`, `e_cross_t +0.0046 mm`,
`e_e2e_t +0.0027 mm`로 사실상 없다. 이 결과로 covariance weighting이나 robust loss의 최종 기여를
확정할 수 없다.
...
- A4-preflight는 A2 대비 overall reprojection `0.0195 px`, `e_cross_t 0.0863 mm`, `e_e2e_t 0.0363 mm`
