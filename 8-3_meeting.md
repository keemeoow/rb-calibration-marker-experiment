...
| 3 | 06:18–10:17 | 3-2에서 reprojection error를 쓰는가? 카메라 간 관계도 목적함수에 넣는가? | C2 목적함수 / FK 사용 | ✅ | 메인 optimizer의 camera-to-camera residual은 0개이고, 카메라는 공유 target pose 변수로 결합된다. visual-only 행은 1항, FK-factor 행은 visual+FK 2항이다. [`TABLE1_RESULTS.md`](CP_result/session04/calib_result_table1/TABLE1_RESULTS.md) |
