...
The output schema is consumed by ``calibration_pipeline.external_gt``.  No external
...
from calibration_pipeline.table1 import detect_observations
from calibration_pipeline.path_evaluation import solve_observed_pose
...
def load_state(path: str, method: str, run_index: int) -> PoseState:
    ...
    runs = payload.get("rows", {}).get(method, {}).get("runs")
...
def predict(args) -> dict:
    state = load_state(args.table1_result_json, args.method, args.run_index)
    ...
    return {
        ...
        "blind_gt_read": False,
        "calibration_source": os.path.abspath(args.table1_result_json),
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="Unified ABLATION_TEST_table1_methods.json produced by 05_calibrate.py")
