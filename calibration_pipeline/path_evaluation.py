...
# its measured cube pose into the common robot-base frame, then every
...
def fixed_camera_cube_pair_disagreement(
    ...
    """Return standard pairwise cube-position and orientation disagreement."""
    left = np.asarray(T_base_cube_left, dtype=np.float64)
    right = np.asarray(T_base_cube_right, dtype=np.float64)
