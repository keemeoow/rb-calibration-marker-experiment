...
**산출물** `CP_result/session01/components/C1/` : `joint_ablation_summary.{csv,json}`, `fk_prior_rotation_fix.json`,
...
**산출물** `CP_result/session01/components/C2/` : `C2_cube_vs_board.{md,csv,json}` (mode 비교) + **`C2_observability.csv`**.
...
[`CP_result/session01/artifacts/fk_cube_artifact`](CP_result/session01/artifacts/fk_cube_artifact/)에 분리되어 있고 비교 파일은
...
python3 CP_table1_ablation.py \
  ...
  --intrinsics_dir intrinsics \
  --calib_dir data/session/calib_out \
  --out_dir CP_result/session01/main/ablation_7row_canonical \
  --num_inits 3 \
  --max_nfev 300 \
  --tol 1e-8
```
...
산출물은 [`CP_result/session01/main/ablation_7row_canonical`](CP_result/session01/main/ablation_7row_canonical/)에 있다.
...
python3 CP_ablation_multisplit.py \
  --out_dir CP_result/session01/validation/ablation_multisplit \
...
[`multisplit_ablation.md`](CP_result/session01/validation/ablation_multisplit/multisplit_ablation.md)에 있으며,
...
python3 CP_synthetic_7row.py \
  --out_dir CP_result/shared/synthetic_7row \
...
[`CP_result/shared/synthetic_7row`](CP_result/shared/synthetic_7row/)에 있다.
...
python3 CP_metric_board_only.py \
  ...
  --out_dir CP_result/shared/metric_board_only
```
...
per-camera 값과 provenance는 [`CP_result/shared/metric_board_only`](CP_result/shared/metric_board_only/)에 있다.
...
python3 CP_sensitivity_7row.py \
  --root_folder data/session \
  --intrinsics_dir intrinsics \
  --out_dir CP_result/session01/validation/sensitivity_7row
```
...
[`CP_result/session01/validation/sensitivity_7row`](CP_result/session01/validation/sensitivity_7row/)에 있다.
...
python3 CP_solver_01_04.py \
  ...
  --intrinsics_dir intrinsics \
  --test_sets 0,4,6,12 \
  --out_dir CP_result/session01/components/solver_01_04
```
...
[`CP_result/session01/components/solver_01_04`](CP_result/session01/components/solver_01_04/)에 있다.
...
PYTHONPATH= python3 CP_D1_fk_correction_2x2.py \
    --root_folder data/session --intrinsics_dir intrinsics \
    --calib_dir data/session/calib_out \
    --out_dir CP_result/session01/diagnostics/D1_fk_correction_2x2
...
**산출물** `CP_result/session01/diagnostics/D1_fk_correction_2x2/` : `D1_fk_correction_2x2.{md,json,csv}`.
...
**산출물** `CP_result/session01/components/C3/` : `ablation_summary.{csv,json}`, `<method>__<prior_mode>/`,
...
### C1 (`CP_result/session01/components/C1`)
...
### C2 (`CP_result/session01/components/C2`)
...
### C3 (`CP_result/session01/components/C3`) — held-out FK 위치 RMSE (핵심 지표)
...
#### ⚠️ 이 순위는 반복 검증되지 않았다 (LOSO, `CP_result/session01/validation/validate_loso`)
...
C3 는 네 가지 solve 를 모두 돌려 `CP_result/session01/components/C3/<method>__<prior>/T_base_C*.npy` 로 각각 저장한다:
...
> (`CP_result/session01/components/C3/04_direct_reprojection_opt__without_robot_cube_prior/T_base_C*.npy`),
...
2. **`+fk`/`+se3` 보정은 LOSO 반복 검증에서도 견고** — 단일 split 11.5~13.4mm → 2.5~2.8mm 를
   ...
   전 fold 재현**된다(`CP_result/session01/validation/validate_loso/fig_CP_C1_loso.png`). raw 는 fold 간 ±5~7mm 로
...
논문용 실데이터 Figure는 `CP_viz_c1_fk_correction.py`가 `CP_result/session01/figures/` 에 세 장으로
생성한다: `fig_CP_C1_fk_correction.png`, `fig_CP_C1_internal_metrics.png`,
`fig_CP_C1_C3_interpretation.png` (+ `fig_CP_C1_real_data_provenance.json`). set별 x/y/z residual 원자료가 summary CSV/JSON에 없으므로
...
1. 🔴 **`meta.json` 의 `set_cube_center_6dof` 회전 규약을 촬영 단계에서 고칠 것** — 지금은
   ...
   `CP_result/session01/components/C3/corrected_set_cube_center_6dof.json` 에 관측에서 복원한 값이 있다.
...
(`Simul_test/figures/*.json`, `CP_result/session01/components/*/*.csv`)만 읽어 생성한다. 의존성은 `numpy matplotlib`
...
**산출물** `CP_result/shared/figures/` (9장) + `CP_result/shared/SIM_vs_CP_summary.{csv,md}` :
