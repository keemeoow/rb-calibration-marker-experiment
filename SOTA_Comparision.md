...
| --- | --- | --- | --- | --- | --- | --- |
| C1 | Tsai–Lenz | classical `AX=XB` | 아니오 | 아니오 | 없음 | 필수 |
| C2 | Park–Martin | classical `AX=XB` | 아니오 | 아니오 | 없음 | 필수 |
| C3 | Horaud | classical `AX=XB` | 아니오 | 아니오 | 없음 | 필수 |
| C4 | Daniilidis | classical dual quaternion | 아니오 | 아니오 | 없음 | 필수 |
| D1 | Shah | robot-world/hand-eye `AX=YB` | 아니오 | 아니오 | 없음 | 필수 |
| D2 | Tabb–Ahmad Yousef | iterative robot-world/hand-eye(s) | 지원 | cost variant에 따름 | 명시적 covariance soft factor 없음 | 필수 |
| D3 | Allegro et al. | reprojection 기반 multi-camera hand-eye | 예 | 예 | robot pose를 고정 입력으로 사용 | 최우선 recent baseline |
| D4 | Calib3R | targetless 3D foundation model | 예 | 2D/3D scene terms | robot motion 기반 calibration term | 조건부 |
| D5 | Ha | probabilistic `AX=YB` | 아니오 | 아니오 | 측정별 noise/reliability 모델 | 필수 FK-uncertainty baseline |
| D6 | Ulrich–Hillemann | uncertainty-aware hand–eye | 아니오 | 지원 구성에 따름 | robot uncertainty와 corrected robot poses | 최우선 FK-uncertainty baseline |
...
Soft-FK 또는 robot uncertainty 자체를 최초 기여로 주장하지 않는다. Strobl–Hirzinger(2006), Ha(2023), Ulrich–Hillemann(2024)이 stochastic 또는 uncertainty-aware hand–eye calibration을 이미 다룬다. 제안 방법의 차별점은 이러한 불확도 처리를 mixed eye-in-hand/eye-to-hand, raw pixel-level, cube+board multi-camera U-BA의 shared latent cube poses에 결합하는 데 둔다.
