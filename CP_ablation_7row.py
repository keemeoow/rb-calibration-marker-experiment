...
* B1/A3/B2 consume one byte-identical, train-only aligned FK cube artifact;
* the primary metric is event-grouped, set-stratified held-out corner
  reprojection with every fitted transform frozen;
* a noise-free A1=A2 and B1=A3 test must pass before real data are fitted.
...
def serialize_state(state: PoseState) -> dict:
    return {
        "T_base_Ci": {str(ci): T.tolist() for ci, T in sorted(state.cams.items())},
        "T_gripper_cam": state.gtc.tolist(),
        "T_base_board": None if state.board is None else state.board.tolist(),
        "T_base_cube_by_set": {str(s): T.tolist() for s, T in sorted(state.cubes.items())},
    }
...
def reprojection_metrics(observations: Sequence[PixelObs], state: PoseState,
                         robot_T, K_map, D_map, gripper: int) -> dict:
    groups: Dict[str, List[float]] = defaultdict(list)
    counts: Dict[str, Dict[str, int]] = defaultdict(lambda: {"observations": 0, "corners": 0})
    unsupported = []
    for obs in observations:
        role = "eih" if int(obs.cam) == int(gripper) else "e2h"
        camera = f"cam_{int(obs.cam)}"
        keys = (
            "overall", obs.marker, role, f"{obs.marker}_{role}", camera,
            f"{obs.marker}_{camera}", f"{role}_{camera}",
            f"{obs.marker}_{role}_{camera}",
        )
        try:
            target = state.board if obs.marker == "board" else state.cubes[int(obs.set_idx)]
            if target is None:
                raise KeyError
            T_base_cam = (np.asarray(robot_T[int(obs.event)], float) @ state.gtc
                          if role == "eih" else state.cams[int(obs.cam)])
            pred = project_points(inv_T(T_base_cam) @ target, obs.object_points,
                                  K_map[int(obs.cam)], D_map[int(obs.cam)])
        except (KeyError, TypeError):
            unsupported.append({"event": int(obs.event), "cam": int(obs.cam),
                                "marker": obs.marker, "set": obs.set_idx})
            continue
        squared = np.square(pred - np.asarray(obs.image_points).reshape(-1, 2)).reshape(-1)
        for key in keys:
            groups[key].extend(squared.tolist())
            counts[key]["observations"] += 1
            counts[key]["corners"] += len(pred)
    result = {}
    for key in sorted(groups):
        result[key] = {
            "rmse_px": float(np.sqrt(np.mean(groups[key]))),
...
def build_fk_artifact_comparison(canonical_artifact: dict,
                                 board_gtc: np.ndarray,
                                 board_alignment_diag: dict) -> dict:
    """Supplementary board-leakage measurement; never consumed by a row."""
    board_delta = np.asarray(
        board_alignment_diag["T_set_cube_center_to_object"], dtype=np.float64)
    canonical_delta = np.asarray(
        canonical_artifact["T_fk_cube_center_to_tag_object"], dtype=np.float64)
    canonical_gtc = np.asarray(canonical_artifact["T_gripper_cam"], dtype=np.float64)
    delta_t, delta_r = pose_delta(canonical_delta, board_delta)
    gtc_t, gtc_r = pose_delta(canonical_gtc, np.asarray(board_gtc, dtype=np.float64))
    body = {
        "artifact_schema": "board_free_vs_board_derived_fk_alignment_comparison_v1",
        "supplementary_only": True,
        "consumed_by_ablation_rows": False,
        "canonical_artifact_sha256": canonical_artifact["artifact_sha256"],
        "canonical_method": canonical_artifact["estimation_method"],
        "board_derived_method": (
            "board_handeye_then_visual_cube_pose_then_robust_SE3_FK_alignment"),
        "canonical_vs_board_derived_fk_delta_translation_mm": delta_t,
        "canonical_vs_board_derived_fk_delta_rotation_deg": delta_r,
        "canonical_vs_board_derived_gtc_translation_mm": gtc_t,
        "canonical_vs_board_derived_gtc_rotation_deg": gtc_r,
        "T_canonical_fk_cube_center_to_tag_object": canonical_delta.tolist(),
        "T_board_derived_fk_cube_center_to_tag_object": board_delta.tolist(),
        "T_canonical_gripper_cam": canonical_gtc.tolist(),
        "T_board_derived_gripper_cam": np.asarray(board_gtc).tolist(),
    }
    encoded = json.dumps(body, sort_keys=True, separators=(",", ":")).encode("utf-8")
    body["comparison_sha256"] = hashlib.sha256(encoded).hexdigest()
    return body


def make_initial_state(condition: AblationCondition, train_obs: Sequence[PixelObs],
                       gripper: int, robot_T, K_map, D_map,
                       board_gtc: np.ndarray, board_initial: np.ndarray,
                       visual_cubes: Mapping[int, np.ndarray],
                       fixed_cubes: Mapping[int, np.ndarray],
                       fixed_gtc_initial: np.ndarray) -> Tuple[PoseState, dict]:
    relevant = filter_observations(train_obs, condition, None, gripper)
    eih = filter_observations(train_obs, condition, "eih", gripper)
    if condition.fk_to_cube == "FK-fixed":
        gtc = np.asarray(fixed_gtc_initial, dtype=np.float64)
        board = (average_board_with_gtc(eih, gtc, robot_T, K_map, D_map, gripper)
                 if "board" in condition.target_set else None)
        cubes = {int(s): np.asarray(T, float) for s, T in fixed_cubes.items()}
        gtc_source = "shared_board_free_FK_artifact_T_gripper_cam"
    else:
        gtc = np.asarray(board_gtc, float)
        board = (np.asarray(board_initial, float) if "board" in condition.target_set else None)
        cubes = ({int(s): np.asarray(T, float) for s, T in visual_cubes.items()}
                 if "cube" in condition.target_set else {})
        gtc_source = "train_eih_board_handeye"
    fixed = filter_observations(relevant, condition, "e2h", gripper)
    cams, cam_sources = estimate_fixed_camera_initials(fixed, board, cubes, K_map, D_map, gripper)
    if not cams:
        raise RuntimeError(f"{condition.row}: no fixed camera can be initialized")
    return PoseState(cams=cams, gtc=gtc, board=board, cubes=cubes), {
        "T_gripper_cam_source": gtc_source,
...
def run_condition_once(condition: AblationCondition, initial_state: PoseState,
    ...
    train_metrics = reprojection_metrics(relevant_train, final_state, robot_T, K_map, D_map, gripper)
    test_metrics = reprojection_metrics(relevant_test, final_state, robot_T, K_map, D_map, gripper)
...
def solve_synthetic(condition: AblationCondition, observations, truth: PoseState,
    ...
    if condition.unified == "seq":
        ...
        final, d = solve_stage(
            ...
            max_nfev=int(args.max_nfev), tol=float(args.tol))
        diag = {"joint": d}
    metric = reprojection_metrics(relevant, final, robot_T, K, D, gripper)
...
def run_noise_free_sanity(args) -> dict:
    ...
    truth_recovery = {}
    passed = True
    for seq_row, unified_row, include_cubes in (("A1", "A2", True), ("B1", "A3", False)):
...
class PreparedAblationData:
    ...
    board_alignment_diagnostics: dict
    alignment_artifact: dict
    alignment_comparison: dict
...
def prepare_ablation_data(args) -> PreparedAblationData:
    ...
    raw_fk_all = s3.load_nominal_set_cube_transforms(train_meta)
    ...
    _, board_alignment_diag = align_fk_from_train(
        train_meta, sorted(eligible), visual_cubes)
    comparison = build_fk_artifact_comparison(
        artifact, board_gtc, board_alignment_diag)
    return PreparedAblationData(
        ...
        board_alignment_diagnostics=board_alignment_diag,
...
def write_outputs(result: dict, out_dir: str) -> None:
    ...
    with open(os.path.join(out_dir, "seven_row_ablation.json"), "w") as handle:
    ...
    for row, entry in result.get("rows", {}).items():
        ...
        def values(section: str, group: str, field: str = "rmse_px") -> List[float]:
            return [float(run[section][group][field]) for run in runs
                    if run.get(section, {}).get(group, {}).get(field) is not None]

        def path_values(field: str) -> List[float]:
            return [float(run["heldout_path_metrics"][field]) for run in runs
                    if run["heldout_path_metrics"].get(field) is not None]

        def mean_std(items: Sequence[float]) -> Tuple[Optional[float], Optional[float]]:
            return ((None, None) if not items else
                    (float(np.mean(items)), float(np.std(items))))

        held_mean, held_std = mean_std(values("heldout_reprojection", "overall"))
        cube_mean, cube_std = mean_std(values("heldout_reprojection", "cube"))
        board_mean, board_std = mean_std(values("heldout_reprojection", "board"))
        eih_mean, eih_std = mean_std(values("heldout_reprojection", "eih"))
        e2h_mean, e2h_std = mean_std(values("heldout_reprojection", "e2h"))
        train_mean, train_std = mean_std(values("train_reprojection", "overall"))
        e2e_t_mean, e2e_t_std = mean_std(path_values("e_e2e_translation_rmse_mm"))
        e2e_r_mean, e2e_r_std = mean_std(path_values("e_e2e_rotation_rmse_deg"))
        cross_mean, cross_std = mean_std(path_values("e_cross_translation_rmse_mm"))
        cross_r_mean, cross_r_std = mean_std(path_values("e_cross_rotation_rmse_deg"))
        n_converged = sum(bool(run["converged"]) for run in runs)
        status = ("converged" if n_converged == len(runs) else
                  ("mixed" if n_converged else "unstable"))
        rows.append({
            "row": row,
            "target_set": entry["condition"]["target_set"],
            "unified": entry["condition"]["unified"],
            "fk_to_cube": entry["condition"]["fk_to_cube"],
            "fk_to_board": entry["condition"]["fk_to_board"],
            "status": status,
            "converged_runs": n_converged,
            "total_runs": len(runs),
            "n_registered_cams": entry["n_registered_cams"],
            "heldout_reprojection_mean_px": held_mean,
            "heldout_reprojection_std_px": held_std,
            "heldout_cube_reprojection_mean_px": cube_mean,
            "heldout_cube_reprojection_std_px": cube_std,
            "heldout_board_reprojection_mean_px": board_mean,
            "heldout_board_reprojection_std_px": board_std,
            "heldout_eih_reprojection_mean_px": eih_mean,
            "heldout_eih_reprojection_std_px": eih_std,
            "heldout_e2h_reprojection_mean_px": e2h_mean,
            "heldout_e2h_reprojection_std_px": e2h_std,
            "train_reprojection_mean_px_diagnostic": train_mean,
            "train_reprojection_std_px_diagnostic": train_std,
            "heldout_e2e_translation_mean_mm": e2e_t_mean,
            "heldout_e2e_translation_std_mm": e2e_t_std,
            "heldout_e2e_rotation_mean_deg": e2e_r_mean,
            "heldout_e2e_rotation_std_deg": e2e_r_std,
            "heldout_cross_translation_mean_mm": cross_mean,
            "heldout_cross_translation_std_mm": cross_std,
            "heldout_cross_rotation_mean_deg": cross_r_mean,
            "heldout_cross_rotation_std_deg": cross_r_std,
        })
        for camera in all_camera_ids:
            camera_mean, camera_std = mean_std(
                values("heldout_reprojection", f"cam_{camera}"))
            rows[-1][f"heldout_cam_{camera}_reprojection_mean_px"] = camera_mean
            rows[-1][f"heldout_cam_{camera}_reprojection_std_px"] = camera_std
    with open(os.path.join(out_dir, "seven_row_ablation.csv"), "w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0]) if rows else ["row"])
        writer.writeheader()
        writer.writerows(rows)
    lines = [
        "# Seven-row ablation",
        "",
        f"Primary metric: `{PRIMARY_METRIC}`. Train reprojection is diagnostic only.",
        "",
        f"Noise-free sanity gate: **{'PASS' if result['noise_free_sanity']['passed'] else 'FAIL'}**.",
        "",
    ...
    for item in rows:
        ...
        lines.append(
            f"| {item['row']} | {item['target_set']} | {item['unified']} | "
            f"{item['fk_to_cube']} | {item['fk_to_board']} | "
            f"{item['status']} ({item['converged_runs']}/{item['total_runs']}) | "
            f"{item['n_registered_cams']} | "
            f"{fmt_pair(item['heldout_reprojection_mean_px'], item['heldout_reprojection_std_px'])} | "
            f"{fmt_pair(item['train_reprojection_mean_px_diagnostic'], item['train_reprojection_std_px_diagnostic'])} |"
        )
    lines += [
        "",
        "Held-out reprojection uses frozen parameters and an event-grouped, set-stratified split. "
        "It is not the whole-position FK-proxy metric. Non-converged rows must not be copied into Table 1 as final numbers.",
        "",
        "## Model-independent path metrics",
        "",
        f"Mask SHA-256: `{result['protocol']['model_independent_path_evaluation_mask']['evaluation_mask_sha256']}`. "
        f"Every cube-bearing row evaluates all "
        f"{len(result['protocol']['model_independent_path_evaluation_mask']['cross_pairs'])} e_cross pairs and "
        f"{len(result['protocol']['model_independent_path_evaluation_mask']['e2e_units'])} e_e2e units; "
        "there is no output-dependent 30 mm/10 degree gate.",
        "",
    ...
    for item in rows:
        lines.append(
            f"| {item['row']} | "
            f"{fmt_pair(item['heldout_cube_reprojection_mean_px'], item['heldout_cube_reprojection_std_px'])} | "
            f"{fmt_pair(item['heldout_board_reprojection_mean_px'], item['heldout_board_reprojection_std_px'])} | "
            f"{fmt_pair(item['heldout_e2e_translation_mean_mm'], item['heldout_e2e_translation_std_mm'])} / "
            f"{fmt_pair(item['heldout_e2e_rotation_mean_deg'], item['heldout_e2e_rotation_std_deg'])} | "
            f"{fmt_pair(item['heldout_cross_translation_mean_mm'], item['heldout_cross_translation_std_mm'])} / "
            f"{fmt_pair(item['heldout_cross_rotation_mean_deg'], item['heldout_cross_rotation_std_deg'])} |"
        )
    lines += [
        "",
        "## Held-out reprojection by camera",
        "",
        "| Row | " + " | ".join(f"cam {camera} (px)" for camera in all_camera_ids) + " |",
        "| --- | " + " | ".join("---:" for _ in all_camera_ids) + " |",
    ]
    for item in rows:
        values_by_camera = [
            fmt_pair(
                item[f"heldout_cam_{camera}_reprojection_mean_px"],
                item[f"heldout_cam_{camera}_reprojection_std_px"])
            for camera in all_camera_ids
        ]
        lines.append(f"| {item['row']} | " + " | ".join(values_by_camera) + " |")
    lines += [
        "",
        f"A separate whole-position result without external ground truth must be labelled "
        f"`{TASK_POSE_PROXY_LABEL}` and must not be described as absolute physical accuracy.",
        "",
    ]
    with open(os.path.join(out_dir, "seven_row_ablation.md"), "w") as handle:
        handle.write("\n".join(lines))


def parse_args():
    parser = argparse.ArgumentParser(description="Fair A0-B3 seven-row reprojection ablation")
...
def main() -> None:
    args = parse_args()
    validate_main_runner_contract()
    print("[SANITY] noise-free A1=A2 and B1=A3")
    ...
    board_gtc, board_initial = prepared.board_gtc, prepared.board_initial
    handeye_diag = prepared.handeye_diagnostics
    visual_cubes, fixed_cubes = prepared.visual_cubes, prepared.fixed_cubes
    fixed_gtc_initial = prepared.fixed_gtc_initial
    artifact = prepared.alignment_artifact
    alignment_comparison = prepared.alignment_comparison
    ...
    with open(comparison_path, "w") as handle:
        json.dump(_jsonable(alignment_comparison), handle, indent=2)
    ...
    result = {
        ...
        "supplementary_fk_artifact_comparison": {
            "path": comparison_path,
            "sha256": alignment_comparison["comparison_sha256"],
            "consumed_by_rows": False,
        },
        "rows": {},
    }
    for row in requested:
        ...
        initial_state, init_diag = make_initial_state(
            condition, train_obs, gripper, robot_T, K_map, D_map,
            board_gtc, board_initial, visual_cubes, fixed_cubes,
            fixed_gtc_initial)
        runs = []
    ...
    print(f"[SAVE] {args.out_dir}/seven_row_ablation.{{json,csv,md}}")
