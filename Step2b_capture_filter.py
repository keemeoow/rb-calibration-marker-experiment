...
def _board_records(session_root: Path, meta: dict, image_scale: float,
...
def _write_readme(path: Path, manifest: dict, retakes: Sequence[dict],
    ...
    lines = [
        ...
        "",
        f"![Step2b review overlay]({OUTPUT_NAMES['overlay']})",
        "",
        "## Standard 제외 cube 관측",
        "",
...
def run_filter(args) -> dict:
    ...
    board_records = _board_records(
        session_root, meta, float(args.image_scale), policies)
    ...
    cube_config = _jsonable(asdict(cube_cfg))
    manifest = {
        ...
        "policies": policies,
        "scope": {
            "cube": "saved non-gripped RGB frames; matches current Step3 contract",
