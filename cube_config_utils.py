...
def infer_cube_config_from_legacy_meta(root_folder: str, default_cfg: Optional[CubeConfig] = None, meta: Optional[dict] = None):
    """Removed by design. Do not infer physical geometry from legacy captures."""
    return None, {"reason": "legacy_inference_removed"}
...
def load_cube_config_from_calibration_summary(calib_dir: str, default_cfg: Optional[CubeConfig] = None) -> Tuple[Optional[CubeConfig], str]:
    ...
    try:
        ...
        return None, "missing"
    if not isinstance(summary.get("cube_config_used"), dict):
        return None, "missing"
    try:
        cfg = cube_config_from_dict(summary["cube_config_used"], default_cfg or get_default_cube_config())
        return cfg, str(summary.get("cube_config_source", "calibration_summary"))
    except Exception:
        return None, "missing"
