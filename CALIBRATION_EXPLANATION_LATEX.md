...
이 방식의 장점은 하한이 데이터를 따라 움직인다는 것이다. 카메라가 깨끗하면 하한도 작아져 gate가 엄격해지고, 노이즈가 크면 하한도 커져 관대해진다. 실제 시뮬레이션에서 깨끗한 조건의 하한은 약 $0.5\,\mathrm{mm}$, 노이즈가 큰 조건에서는 약 $2\,\mathrm{mm}$로 잡힌다.
