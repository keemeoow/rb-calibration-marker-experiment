...
따라서 모든 관측을 공통 기준인 로봇 베이스 좌표계 $B$로 옮겨야 한다. 그래야 여러 카메라가 같은 물체에 대해 같은 좌표를 말할 수 있다.

### 1.4 이 프로젝트의 설정

이 프로젝트는 로봇 그리퍼가 큐브를 잡고 여러 위치에 옮겨 놓으면서 관측을 모은다. 큐브를 한 번 놓은 상태를 세트라고 부른다.
...
로봇이 큐브를 잡고 있으므로 로봇의 관절값으로부터 큐브가 어디 있는지 짐작할 수 있다. 이 정보를 얼마나 믿을지가 이 문서 후반부의 핵심 주제다.
...
- 큐브의 진짜 위치: 베이스 기준 $100\,\mathrm{mm}$
- 카메라의 진짜 위치: 베이스 기준 $0\,\mathrm{mm}$

카메라가 정직하다면 $\mathbf{Z}=100$이라고 보고해야 한다.

**여기에 두 가지 계통오차가 있다.**

- FK 오차: 로봇은 큐브가 $105$에 있다고 말한다. $5\,\mathrm{mm}$ 초과다.
- 카메라 오차: 카메라는 큐브가 $103$ 거리에 있다고 본다. $3\,\mathrm{mm}$ 초과다.

**Fixed-FK의 계산.** $\mathbf{Q}_s$는 FK 값 $105$에 못 박혀 움직이지 않는다. 따라서 $\mathbf{C}\mathbf{Z}=\mathbf{Q}_s$를 만족하도록 카메라 위치를 정한다.
...
**여기서 핵심은 최적화가 본 정보의 양이다.** 최적화가 아는 것은 $105$와 $103$의 차이인 $2$라는 숫자 하나뿐이다. 그런데 그 $2$의 정체는 다음과 같다.

$$
2 = \underbrace{5}_{\text{FK가 초과한 몫}} - \underbrace{3}_{\text{카메라가 초과한 몫}}
$$

최적화는 이 분해를 볼 수 없다. $5$와 $3$이든 $7$과 $5$든 $2$와 $0$이든 모두 같은 $2$를 만들고, 목적함수 안에는 셋을 구별할 정보가 없다. 만약 카메라 오차가 반대 방향이었다면 $\mathbf{C}=8$이 되어 훨씬 나빠졌을 것이다. 즉 두 오차는 우연히 상쇄될 수도 있고 겹쳐서 커질 수도 있는데, 어느 쪽인지 알 방법이 없다.

**구조적으로 분리가 불가능한 이유.** 카메라의 계통오차가 카메라 좌표계에서의 일정한 어긋남이라면, 그것은 카메라가 다른 곳에 있다고 말하는 것과 수학적으로 같다. 즉 $\mathbf{C}_i$를 바꾸는 것과 구별되지 않는다. 최적화 입장에서 두 오차는 애초에 별개의 대상이 아니라, 하나의 미지수에 두 원인이 합쳐져 들어온 것이다.

**`No-FK`와 비교하면 차이가 분명해진다.** `No-FK`는 큐브 FK 자세를 쓰지 않으므로 FK 오차가 들어올 통로가 없고, 오염원이 카메라 하나뿐이다. 반면 `Fixed-FK`는 FK 오차와 카메라 오차가 모두 같은 자리인 $\mathbf{C}_i$와 $\mathbf{X}$로 들어온다. 카메라 계통오차는 어느 방법도 잡지 못하는 공통 약점이지만, `Fixed-FK`는 거기에 FK 오차까지 같은 주머니에 넣는다는 점이 추가로 나쁘다.

**더 곤란한 것은 잔차가 이를 알려주지 않는다는 점이다.** 위 계산에서 $\mathbf{C}=2$를 넣으면 $2+103=105$로 정확히 맞아떨어져 잔차가 $0$이 된다. 수렴은 완벽해 보이지만 카메라 위치는 $2\,\mathrm{mm}$ 틀려 있다. 재투영오차는 카메라 자기 자신과의 일치도이므로 이 편향에 둔감하고, 여러 카메라가 같은 모델과 같은 절차로 캘리브레이션되었다면 비슷한 방향으로 치우쳐 카메라 간 consistency도 통과한다. 18.2절이 FK 일치도를 절대 정확도로 읽지 말라고 경고하는 이유가 여기에 있다.

다만 카메라 오차가 거리나 각도에 따라 달라지는 종류라면 강체 어긋남이 아니므로 $\mathbf{C}_i$ 하나로 완전히 흡수되지 않고 잔차가 조금 남는다. 그러나 그 잔차만으로 카메라 탓인지 FK 탓인지 가려내기는 여전히 어렵다.
...
강건 평균이 이상치를 걸러내는 기준은 다음과 같다. 현재 평균을 기준으로 각 예측이 얼마나 떨어져 있는지를 재고, 그 거리들의 분포에서 잘라낼 선을 정한다.
...
걸러낸 뒤 남은 값으로 평균을 다시 내고, 더 걸러낼 것이 없을 때까지 이 과정을 최대 세 번 반복한다. 이상치를 한 번 제거하면 평균이 조금 옮겨가고, 그 새 평균을 기준으로 다시 재면 처음에 놓쳤던 이상치가 드러나기 때문이다.
...
2. 보정은 $\overline{\boldsymbol{\Delta}}$만큼 해주므로, 어긋난 만큼 정확히 보정받는다. 따라서 $\mathbf{F}^{\mathrm{corr}}_s=\mathbf{V}_s$가 되어 vision이 말한 자리에 정확히 도착한다.
3. 두 값이 같아졌으므로 그 사이 거리인 $d_t(s)$는 $0$이다.

시계로 비유하면, 사무실 시계들이 평균 $5$분씩 빠를 때 모든 시계에서 $5$분을 빼는 보정을 하는 상황이다. 정확히 $5$분 빨랐던 시계는 보정 후 딱 맞아서 오차가 $0$이 되고, $8$분 빨랐던 시계는 $3$분이 남는다.

여기서 중요한 점은 $d_t(s)=0$이 FK가 정확했다는 뜻이 아니라는 것이다. 위 시계도 원래는 $5$분이나 틀렸었다. $d_t(s)$가 $0$이라는 것은 남들만큼만 틀렸다는 뜻이다.
