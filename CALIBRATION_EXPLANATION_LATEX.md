...
$\mathbf{O}_s$는 세트 $s$에서 큐브가 베이스 기준으로 놓여 있는 자세다. `No-FK`에서는 이 값을 아무도 알려주지 않으므로, 최적화가 직접 찾아낸다. "큐브가 여기 있었다고 치면 모든 카메라의 관측이 가장 잘 맞아떨어진다"는 자세를 고르는 것이다.

여기서 비슷해 보이는 세 기호를 구분해 둔다.
