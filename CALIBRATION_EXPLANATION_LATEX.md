...
- **가중치**: 각 값을 얼마나 믿을지 나타내는 숫자. 카메라 $5$대가 본 세트와 $1$대만 본 세트를 같은 무게로 다루지 않겠다는 뜻이다.
- **강건 평균**: MAD로 이상치를 거른 뒤 남은 값을 가중치대로 평균한다. 뒤에 나오는 $\operatorname{RobustWeightedAverage}$와 $\operatorname{RobustAverage}$가 모두 이 방식이다.
