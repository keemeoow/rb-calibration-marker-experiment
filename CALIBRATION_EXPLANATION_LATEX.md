...
기준선 $\tau_t$와 $\tau_R$은 $d_t(s)$와 $d_R(s)$ 값들이 실제로 어떻게 분포하는지를 보고 정한다.
...
식을 말로 풀면 이렇다. 세트들의 차이값을 쭉 늘어놓고 가운데 값을 찾은 뒤, 거기서 얼마쯤 떨어진 지점까지를 정상으로 인정할지 선을 긋는 것이다.

각 항의 뜻은 다음과 같다.

- $\operatorname{median}_s(d)$: 차이값들의 가운데 값. 보통 이 정도는 차이가 난다는 기준점이다.
- $\operatorname{MAD}_s(d)$: 차이값들이 그 가운데 값 주변에 얼마나 모여 있는지. 11.3절에서 설명한 그 MAD다.
- $1.4826$: MAD를 표준편차와 같은 척도로 바꾸는 환산 상수다. 정규분포에서 MAD에 이 값을 곱하면 표준편차가 된다. 이 환산 덕분에 뒤의 $k$를 "표준편차 몇 배"로 읽을 수 있다.
- $k=2.5$: 가운데 값에서 표준편차 몇 배까지 봐줄지 정하는 숫자다. 크게 잡으면 관대해지고 작게 잡으면 엄격해진다. $2.5$는 정규분포라면 상위 $1\%$ 정도만 잘라내는 세기이며, $\overline{\boldsymbol{\Delta}}$를 구하는 강건 평균이 이미 쓰고 있는 값과 같다.

### 왜 상수가 아니라 분포로 정하는가

$d_t(s)$가 실제로 재는 것이 무엇인지 보면 답이 나온다.

모든 세트의 FK가 똑같은 방향으로 어긋나 있다고 하자. 그 공통된 어긋남은 이미 $\overline{\boldsymbol{\Delta}}$가 걷어냈으므로 $d_t(s)$는 전부 $0$에 가깝게 나온다. 반대로 한 세트만 유별나게 틀어져 있으면 그 세트의 $d_t(s)$만 커진다.

즉 $d_t(s)$는 FK가 얼마나 틀렸는지를 재는 값이 아니다. **이 세트가 다른 세트들과 비교해 얼마나 유별난지**를 재는 값이다.

그런데 "유별나다"의 기준은 나머지 세트들이 얼마나 모여 있느냐에 따라 달라진다. 세트들이 모두 $2\,\mathrm{mm}$ 안에 모여 있다면 $10\,\mathrm{mm}$짜리 세트는 명백히 이상하다. 반대로 원래부터 $30\,\mathrm{mm}$씩 흩어져 있다면 같은 $10\,\mathrm{mm}$은 지극히 평범하다.

고정 상수는 이 두 상황을 구분하지 못한다. 그래서 기준선을 데이터의 산포에서 끌어온다.

### 보호 장치

이 방식에는 보호 장치가 두 개 필요하다.
