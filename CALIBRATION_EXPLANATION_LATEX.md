...
### 9.3 여기서 $\mathbf{O}_s$의 지위

$\mathbf{O}_s$는 세트 $s$에서 큐브가 베이스 기준으로 실제 놓여 있던 자세다. `No-FK`에서는 이 값을 아무도 알려주지 않으므로, 최적화가 직접 찾아낸다. "큐브가 여기 있었다고 치면 모든 카메라의 관측이 가장 잘 맞아떨어진다"는 자세를 고르는 것이다. 위 argmin 아래에 $\mathbf{C}_i$, $\mathbf{X}$와 나란히 $\mathbf{O}_s$가 들어가 있는 이유가 이것이다.

주의할 점은 $\mathbf{O}_s$가 우리가 원하는 답이 아니라는 것이다. 실제로 얻고 싶은 결과는 카메라 자세 $\mathbf{C}_i$와 hand-eye $\mathbf{X}$이며, 큐브가 어디 놓였는지는 실험이 끝나면 쓸 일이 없다. 그러나 8장 목적함수에서 카메라 예측을 비교할 상대가 필요하므로, 원하지 않아도 함께 풀어야 한다. 이렇게 목적이 아니면서 어쩔 수 없이 같이 추정하는 변수를 부수 미지수(nuisance parameter)라고 부른다.

그 대가는 미지수 개수다. 세트마다 $\mathbf{O}_s$가 하나씩 늘고 각각 $6$자유도이므로, 세트가 $10$개면 미지수가 $60$개 추가된다. 카메라 $3$대의 $18$개와 hand-eye의 $6$개를 합한 $24$개보다 훨씬 많다. `Fixed-FK`는 이 $60$개를 통째로 없애는 대신 FK 오차를 그대로 떠안는다. 이 맞교환이 10장부터 12장까지의 핵심이다.
