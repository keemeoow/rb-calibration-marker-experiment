...
여기서 쓰인 세 용어의 뜻은 다음과 같다.

**이상치**는 나머지 값들과 동떨어진 값이다. 코너를 잘못 검출했거나 사진이 흐렸거나 큐브의 면을 다른 면으로 착각하면 생긴다. 그냥 평균을 내면 이런 값 하나가 결과를 통째로 끌고 간다. 예를 들어 예측 $7$개 중 $6$개가 서로 $2\,\mathrm{mm}$ 안에 모여 있는데 하나만 $100\,\mathrm{mm}$ 떨어져 있으면, 단순 평균은 옳은 위치에서 약 $14\,\mathrm{mm}$ 밀려난다.

**MAD**는 Median Absolute Deviation, 우리말로 중앙값 절대편차의 줄임말이다. 값들이 얼마나 흩어져 있는지 재는 자인데, 이상치에 흔들리지 않는다는 점이 표준편차와 다르다. 구하는 방법은 두 단계다. 먼저 값들의 중앙값을 구하고, 다음으로 각 값이 그 중앙값에서 얼마나 떨어졌는지를 잰 뒤 그 거리들의 중앙값을 취한다. 중앙값은 극단값 하나가 끼어도 거의 움직이지 않으므로, 이렇게 잰 흩어짐 역시 이상치에 오염되지 않는다. 실제로는 중앙값에서 MAD의 몇 배 이상 떨어진 값을 이상치로 보고 버린다.

**가중치**는 각 값을 얼마나 믿을지를 나타내는 숫자다. 모두 똑같이 반영하는 대신, 믿을 만한 값에 더 큰 몫을 준다. 여기서는 관측이 많이 뒷받침하는 세트일수록 그 합의 자세가 더 안정적이므로 큰 가중치를 준다. 카메라 $5$대가 본 세트와 $1$대만 본 세트를 같은 무게로 다루지 않겠다는 뜻이다.

**강건 평균**은 이 둘을 합친 것이다. MAD로 이상치를 걸러내고, 남은 값들을 가중치대로 평균한다. 뒤에 나오는 $\operatorname{RobustWeightedAverage}$와 $\operatorname{RobustAverage}$가 모두 이 방식이다.
