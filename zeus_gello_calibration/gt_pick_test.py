...
pick 자세 계산은 session2_pick_and_place.py와 같은 관례를 그대로 쓴다 --
검출된 큐브의 x,y,rz만 쓰고 z/ry/rx는 GRASP_REF_POSE 고정값(session2에서
이미 실측 검증됨). 큐브 바닥(접촉면)에서 object-frame origin까지의 높이는
메인 큐브와 GT 큐브가 우연히 동일(둘 다 29.5mm -- 메인 큐브는 57mm 몸체+
origin이 중심보다 1mm 위, GT 큐브는 59mm 정육면체 몸체+origin이 중심 그대로,
계산하면 둘 다 29.5mm)이라 Z_FIXED를 그대로 쓸 수 있다.
...
from session2_pick_and_place import (  # noqa: E402
    Z_FIXED, RY_FIXED, RX_FIXED, APPROACH_MM_DEFAULT, MOVE_LIN_SPEED,
...
def build_pick_target(T_base_cube):
    pose6 = T_to_pose6(T_base_cube)  # [x,y,z,rz,ry,rx] mm/deg -- z/ry/rx는 안 쓰고 고정값으로 대체
    x, y, rz = float(pose6[0]), float(pose6[1]), float(pose6[3])
    return [x, y, Z_FIXED, rz, RY_FIXED, RX_FIXED]
...
def main():
    ...
    try:
        for trial in range(1, args.trials + 1):
            ...
            target = build_pick_target(T_base_cube)
