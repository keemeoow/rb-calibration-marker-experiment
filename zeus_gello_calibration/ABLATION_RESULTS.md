...
>
> **"독립"의 정의는 `독립_no-fk`(진짜 독립: 고정캠 그룹과 그리퍼 그룹이 서로
> 정보 교환 전혀 없이 각자 풀고, 끝난 뒤 사후 합의만 확인)로 확정했다.**
> `sequential_no-fk`(table1.py A1의 공식 알고리즘: 그리퍼가 먼저 풀고 얼리면
> 고정캠이 거기 일방적으로 맞춤)는 C1 검증을 위해 별도로 만든 참고용이며,
> "독립"의 대표값으로 쓰지 않는다.
...
Train RMSE는 학습에 쓴 데이터로 재는 것이고, held-out RMSE는 **2026-09-09
수정본 기준 — 정답을 그 세트를 본 카메라들 자신으로 삼각측량하지 않고,
"그 세트에서 로봇이 실제로 명령받아 간 FK 위치 @ session1의 T_gripper_cube"
로 계산한, 비전과 완전히 무관한 값**을 정답으로 써서 잰 것이다(카메라들의
공통 편향까지 잡아낼 수 있는 더 엄격한 기준으로 바꿈 — 상세 이유는 커밋
`d357cb8` 참고).

| 방식 | Train RMSE (px) | Held-out RMSE (px, FK 기준) | Cross-cam (mm/deg) | 비고 |
|---|---:|---:|---:|---|
| 통합_raw-fk | 1.37 | **2.84** | 4.59 / 1.07 | A3 아님 (아래 주의 참고) |
| sequential_no-fk (참고용) | 1.46 | 3.76 | 4.25 / 1.17 | table1.py A1 공식 알고리즘 그대로, "독립"의 대표값 아님 |
| **통합_no-fk** | **0.93** | 4.28 | **3.71 / 0.97** | |
| **독립_no-fk** (진짜 독립) | 고정캠 1.12 / 그리퍼 0.66 | 4.28 | 4.05 / 0.96 | 사후 합의: 평균 3.18mm/0.95°, 최대 5.57mm/2.45° |
...
3. **통합_no-fk와 독립_no-fk는 이 지표에서 거의 같게 나왔다(4.275 vs 4.278)** — 우연의 일치이지, 두 방법이 사실상 같다는 뜻은 아니다(train RMSE로 보면 여전히 뚜렷이 다르다: 0.93 vs 1.12/0.66, 그리고 독립은 두 그룹이 서로 3.18mm/0.95° 어긋난다는 사후 합의 문제도 있음).
