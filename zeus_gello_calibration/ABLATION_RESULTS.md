...
|---|---:|---:|---:|---:|---|
| 통합_raw-fk | 1.37 | **2.84** | 2.24 / 0.58 | 4.59 / 1.07 | A3 아님 (위 주의 참고) |
| **통합_no-fk** | **0.93** | 4.28 | **2.03 / 0.64** | **3.71 / 0.97** | |
| 독립_no-fk | 고정캠 1.12 / 그리퍼 0.66 | 4.28 | 2.18 / 0.66 | 4.05 / 0.96 | 사후 합의: 평균 3.18mm/0.95°, 최대 5.57mm/2.45° |
...
2. **같은 held-out을 실제 mm로 바꿔서 보면 순위가 다시 뒤집힌다 — 통합_no-fk가 제일 정확하다(2.03mm)**, raw-fk(2.24mm)와 독립(2.18mm)보다 오히려 낫다. 픽셀 공간의 큰 격차(2.84 vs 4.28px)가 실제 3D 위치 오차로는 거의 안 벌어진다(2.0~2.2mm 사이) — 카메라 거리/각도에 따라 같은 픽셀 오차가 다른 mm 오차로 바뀌기 때문에, **픽셀 RMSE만으로 "이 방법이 더 정확하다"고 결론 내리면 안 된다는 걸 보여주는 사례**.
3. **통합_no-fk와 독립_no-fk는 held-out 픽셀 지표에서 거의 같게 나왔다(4.275 vs 4.278)** — 우연의 일치이지, 두 방법이 사실상 같다는 뜻은 아니다(train RMSE로 보면 여전히 뚜렷이 다르다: 0.93 vs 1.12/0.66, mm로도 2.03 vs 2.18로 통합이 근소하게 낫다). 그리고 독립은 두 그룹이 서로 3.18mm/0.95° 어긋난다는 사후 합의 문제도 있다 — 각자는 괜찮아 보여도 서로 안 맞을 수 있다는 걸 직접 보여주는 지표이자, 통합이 왜 필요한지에 대한 가장 직접적인 근거.
...
session1(42)+session3(15)는 항상 전부 포함하고, session2의 15개 placement 중 학습에 쓰는 개수만 30%/50%/70%로 줄여서(나머지는 held-out), 매 비율마다 서로 다른 랜덤 조합으로 8번씩 반복 평균. 코드: `eval_data_efficiency.py`.
...
- **순위가 데이터 비율과 무관하게 일관된다** — 통합_no-fk가 mm 기준 3개 비율 전부에서 1등(2.13~2.18mm). px 기준으로는 raw-fk가 계속 유리(3.0~3.1px vs no-fk 계열 4.0~4.5px)한데, 이 역시 앞서 설명한 것과 같은 이유(raw-fk의 학습 목표=FK=검증 기준)로 보인다.
- **데이터를 30%→70%로 늘려도 mm 오차가 뚜렷이 줄지 않는다** — 오히려 독립_no-fk는 데이터가 늘수록 약간 나빠지는 경향(2.18→2.26→2.31mm)도 보이는데, 비율마다 held-out 세트 개수가 다르고(11→7→5개) 반복 8회의 표준편차도 커서(0.12~0.43mm) 이게 실제 경향인지 표본이 작아서 생긴 우연인지는 불확실하다.
- "데이터가 적을수록 통합의 이점이 커진다"는 가설은 이번 실험에서 뚜렷이 확인되지 않았다 — 다만 어떤 비율에서도 통합이 독립에 밀리지는 않았다.
