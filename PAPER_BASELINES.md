...
## 6. Kalib (IROS 2025) — 학습기반 baseline ① 확정: "AI가 그리퍼 끝점을 추적"
...
## 7. MASt3R-SfM + FK 앵커 — 학습기반 baseline ② 확정: "AI 장면복원 + 로봇 앵커"
...
- 풀이 절차
  ...
  - (같은 문제를 다룬 Calib3R 논문(arXiv:2509.08813, 코드 미공개 → related work 인용만)이 쓴
    자체 베이스라인 **"MASt3R-SfM + Calib" 구성 그대로** — 자의적 조립 아님)
...
- 비교 방법들의 FK 취급: Shah·Tabb·Allegro = **100% 신뢰** / Tsai·Daniilidis = 움직임 비교만 /
  MASt3R-SfM = 정렬 앵커만 / Kalib = 기준점 3D 좌표(정답 레이블)로 사용
- 우리 = **"적당히 믿기" + "반복 오차 지우기"**
...
## 9. 평가: 실제 로봇에서만, "정답을 아는 값" 4가지로 채점 (확정)
...
## 11. 멀티카메라 비교로 만드는 법 — 전부 가능 (그래서 선정)
...
### 부류 2: 원래부터 멀티카메라 (Tabb · Allegro · MASt3R-SfM+앵커)
...
> 참고 (related work 인용 전용): Calib3R(arXiv:2509.08813) 은 코드 미공개(리포 404)로 실험군 제외 (2026-08-05).
> 그 역할은 §7(동일 구성 베이스라인)이 대체. 코드 재공개 시 복귀 검토.
